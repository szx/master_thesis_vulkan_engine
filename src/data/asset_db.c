...
#define create_table(table, value_names, ...)                                                      \
  data_db_create_key_value_table_for_multiple_values(assetDb->db, #table, value_names);
  DATA_ASSET_DB_TABLES(create_table, )
#undef create_table
  data_asset_db_save(assetDb);
}

void data_asset_db_insert_primitive_int(data_asset_db *assetDb, char *key, char *column,
                                        int value) {
  data_db_insert_int(assetDb->db, "primitive", key, column, value);
}

void data_asset_db_insert_primitive_blob(data_asset_db *assetDb, char *key, char *column,
                                         void *value, size_t size) {
  data_db_insert_blob(assetDb->db, "primitive", key, column, value, size);
}
