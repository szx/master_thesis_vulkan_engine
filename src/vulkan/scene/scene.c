...
vulkan_scene *vulkan_scene_create(data_asset_db *assetDb, vulkan_device *vkd,
  ...
  scene->vkd = vkd;
  scene->data = vulkan_data_scene_create_with_asset_db(scene->assetDb, sceneName);
...
void vulkan_scene_destroy(vulkan_scene *scene) {
  vulkan_data_scene_destroy(scene->data);
...
void vulkan_scene_build_geometry_buffer(vulkan_scene *scene) {
  ...
  // HIRO rethink vulkan_scene_build_geometry_buffer: interleave attributes from vulkan_data_node
  // here?
  /*
  ...
  for (size_t nodeIdx = 0; nodeIdx < core_array_count(scene->data->nodes); nodeIdx++) {
    log_debug("node:\n");
    vulkan_data_node *node = &scene->data->nodes.ptr[nodeIdx];
