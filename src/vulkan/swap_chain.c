...
choose_swap_surface_format(vec_VkSurfaceFormatKHR *availableFormats) {
  for (size_t i = 0; i < availableFormats->size; i++) {
    VkSurfaceFormatKHR availableFormat = availableFormats->value[i];
    if (availableFormat.format == VK_FORMAT_B8G8R8A8_SRGB &&
        availableFormat.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
      return availableFormat;
    }
  }
  return availableFormats->value[0];
}

VkPresentModeKHR
choose_swap_present_mode(vec_VkPresentModeKHR *availablePresentModes) {
  for (size_t i = 0; i < availablePresentModes->size; i++) {
    VkPresentModeKHR availablePresentMode = availablePresentModes->value[i];
    if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
      return availablePresentMode;
    }
  }
  return availablePresentModes->value[0];
}
...
void create_swap_chain(vulkan_swap_chain *vks) {
  ...
  VkPresentModeKHR presentMode =
      choose_swap_present_mode(&swapChainInfo->presentModes);
  ...
  vulkan_queue_families queueFamilies =
      find_queue_families(vks->vkd, vks->vkd->physicalDevice);
  uint32_t queueFamiliesIndices[] = {queueFamilies.graphicsFamily,
...
void create_swap_chain_image_views(vulkan_swap_chain *vks) {
  ...
  vec_VkImageView_resize(&vks->swapChainImageViews, vks->swapChainImages.size,
                         VK_NULL_HANDLE);

  for (size_t i = 0; i < vks->swapChainImageViews.size; i++) {
    vks->swapChainImageViews.value[i] = create_image_view(
        vks->vkd, vks->swapChainImages.value[i], VK_IMAGE_VIEW_TYPE_2D,
