...
vulkan_light_state *vulkan_light_state_create(vulkan_render_state *renderState) {
  vulkan_light_state *lights = core_alloc(sizeof(vulkan_light_state));
...
void vulkan_light_state_destroy(vulkan_light_state *lights) {
...
vulkan_light_state_get_directional_light_element(vulkan_light_state *lights, size_t lightIdx) {
  if (utarray_len(lights->directionalLightElements) == 0) {
    // NOTE: Default directional light.
    vec3 direction = {0.0, 0.0, 1.0};
    vec3 color = GLM_VEC3_ONE_INIT;
    vulkan_light_state_add_directional_light(lights, direction, color);
    return vulkan_light_state_get_directional_light_element(lights, lightIdx);
  }
...
void vulkan_light_state_add_directional_light(vulkan_light_state *lights, vec3 direction,
                                              vec3 color) {
...
void vulkan_light_state_set_directional_light_elements(
    vulkan_light_state *lights, uint32_t *directionalLightCount,
...
vulkan_light_state_get_point_light_element(vulkan_light_state *lights, size_t lightIdx) {
  if (utarray_len(lights->pointLightElements) == 0) {
    // NOTE: Default point light.
    vec3 position = GLM_VEC3_ZERO_INIT;
    vec3 color = GLM_VEC3_ONE_INIT;
    float radius = 1.0f;
    vulkan_light_state_add_point_light(lights, position, color, radius);
    return vulkan_light_state_get_point_light_element(lights, lightIdx);
  }
  lightIdx = lightIdx % utarray_len(lights->pointLightElements);
  return utarray_eltptr(lights->pointLightElements, lightIdx);
}

void vulkan_light_state_add_point_light(vulkan_light_state *lights, vec3 position, vec3 color,
                                        float radius) {
...
void vulkan_light_state_set_point_light_elements(
    vulkan_light_state *lights, uint32_t *pointLightCount,
...
void vulkan_light_state_debug_print(vulkan_light_state *lights, int indent) {
