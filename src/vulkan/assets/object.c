...
void vulkan_asset_object_init(vulkan_asset_object *object, vulkan_scene_data *sceneData) {
  object->sceneData = sceneData;
  object->mesh = NULL;
  object->camera = NULL;
  glm_mat4_identity(object->transform);
  utarray_alloc(object->children, sizeof(vulkan_asset_object *));

  VULKAN_ASSET_FIELD_DEFS(object, object)
}

void vulkan_asset_object_deinit(vulkan_asset_object *object) {
  if (object->mesh) {
    vulkan_asset_mesh_deinit(object->mesh);
    core_free(object->mesh);
  }
  if (object->camera) {
    vulkan_asset_camera_deinit(object->camera);
    core_free(object->camera);
  }
  utarray_free(object->children);
}

data_key vulkan_asset_object_calculate_key(vulkan_asset_object *object) {
  hash_t value;
  HASH_START(hashState)
  HASH_UPDATE(hashState, &object->transform, sizeof(object->transform))

  utarray_foreach_elem_deref (vulkan_asset_object *, child, object->children) {
...
void vulkan_asset_object_serialize(vulkan_asset_object *object, data_asset_db *assetDb) {
  object->key = vulkan_asset_object_calculate_key(object);

  if (object->mesh) {
    vulkan_asset_mesh_serialize(object->mesh, assetDb);
    data_asset_db_insert_object_mesh_key(assetDb, object->key, object->mesh->key);
  }

  if (object->camera) {
    vulkan_asset_camera_serialize(object->camera, assetDb);
  ...
  utarray_foreach_elem_deref (vulkan_asset_object *, child, object->children) {
    vulkan_asset_object_serialize(child, assetDb);
...
void vulkan_asset_object_deserialize(vulkan_asset_object *object, data_asset_db *assetDb,
                                     data_key key) {
  object->key = key;
  ...
  if (meshHash.value != 0) {
    object->mesh = core_alloc(sizeof(vulkan_asset_mesh));
    vulkan_asset_mesh_init(object->mesh, object->sceneData);
    vulkan_asset_mesh_deserialize(object->mesh, assetDb, meshHash);
  } else {
  ...
  if (cameraHash.value != 0) {
    object->camera = core_alloc(sizeof(vulkan_asset_camera));
    vulkan_asset_camera_init(object->camera, object->sceneData);
    vulkan_asset_camera_deserialize(object->camera, assetDb, cameraHash);
  } else {
  ...
  utarray_foreach_elem_deref (data_key, childKey, childrenHashArray.values) {
    vulkan_asset_object *child =
        vulkan_scene_data_get_object_by_key(object->sceneData, assetDb, childKey);
    utarray_push_back(object->children, &child);
  }
  data_key_array_deinit(&childrenHashArray);
}

void vulkan_asset_object_debug_print(vulkan_asset_object *object, int indent) {
  ...
  if (object->mesh) {
    vulkan_asset_mesh_debug_print(object->mesh, indent + 2);
  }

  if (object->camera) {
    vulkan_asset_camera_debug_print(object->camera, indent + 2);
  }

  size_t i = 0;
  utarray_foreach_elem_deref (vulkan_asset_object *, child, object->children) {
    log_debug(INDENT_FORMAT_STRING "child #%d", INDENT_FORMAT_ARGS(2), i++);
    vulkan_asset_object_debug_print(child, indent + 2);
  }
}
