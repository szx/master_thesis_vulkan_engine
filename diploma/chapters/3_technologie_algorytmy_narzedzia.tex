...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=2mm]
		...
		\def\x{descriptors};\node (\x) [file, right = 4mm of unified_constant_buffer] {\myesc{\x}}; \edef\y{\x};
		\def\x{shader};\node (\x) [file, below = of \y] {\myesc{\x}}; \edef\y{\x};
...
Obiekty biorące udział w procesie konwersji sceny na polecenia rysowania są przedstawione na poniższym diagramie:
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance=0.5cm]
		\tikzstyle{entity} = [rectangle, minimum width=3cm, minimum height=0.5cm,text centered, draw=black]
	...
	\end{tikzpicture}
	\caption{Obiekty biorące udział w procesie konwersji sceny na polecenia rysowania (opracowanie własne)}
	\label{scene_process}
\end{figure}
