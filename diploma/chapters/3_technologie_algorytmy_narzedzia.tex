...
Silnik używa kodu w języku C wygenerowanego przez automatyczny generator kodu, który jest skryptem Python uruchamianym przez CMake przed rozpoczęciem reszty kompilacji.

// HIRO sources: config, headers

// HIRO globals: wygenerowane stałe
// HIRO X makra
...
\paragraph{Jednostka thirdparty}
...
Komunikaty są zapisywane do standardowego wyjścia (\textit{stdout} albo \textit{stderr}) oraz do pliku tekstowego na dysku, którego nazwa zaostała zdefiniowana w zmiennych globalnych generatora kodu (domyślnie \textit{log.txt}).
