...
Jednostka definiuje też makra używające formy metaprogramowania w celu dodania nowych struktur kontrolnych \cite{METACONTROLC} upraszczających iterowanie po strukturach danych bibliteki \textit{uthash}, której użycie zostało przedstawione na listingu \ref{junk4}.
\lstset{language=C}
...
Obiekty biorące udział w procesie konwersji sceny są przedstawione na poniższym diagramie:
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance=0.5cm]
		\tikzstyle{entity} = [rectangle, minimum width=3cm, minimum height=0.5cm,text centered, draw=black]
...
Główny moduł silnika odpowiedzialny za stworzenie stanu renderowania oraz grafu renderowania oraz implementację używającej ich pętli głównej programu.

\subsubsection{Pamięć podręczna renderera renderer\_cache}
Pamięć podręczna renderera \textit{renderer\_cache} jest niskopoziomową reprezentacją sceny pośredniczącą pomiędzy modułami sceny i renderara.

Obiekt jest złożony z list elementów \textit{renderer\_cache\_*\_element}:
\begin{itemize}
	\item \textit{renderer\_cache\_primitive\_element}: prymityw HIRO;
	\item \textit{renderer\_cache\_camera\_element}: HIRO;
	\item \textit{renderer\_direct\_light\_element}: HIRO;
	\item \textit{renderer\_skybox\_element}: HIRO;
	\item \textit{renderer\_font\_element}: HIRO.
\end{itemize}

Inicjalizację obiektu mozna podzielić na dwie fazy:
\begin{itemize}
	\item graf sceny wypełniania pamięć podręcznej renderara informacjami o scenie,
	\item pamięć podręczna renderara wypełnia stan renderara zasobami i poleceniami rysowania.
\end{itemize}



Stan każdego elementu można podzielić na dwie cześci:
\begin{itemize}
	\item stan CPU: wypełniony od razu po dodaniu elementu przez graf sceny metodą \textit{add\_*\_element()}.,
	\item stan GPU: wypełniony po aktualizacji stanu GPU renderara.
\end{itemize}
...
\begin{itemize}
	...
	\item Metoda \textit{add\_new\_primitive\_elements\_to\_batches()} dodaje polecenia rysowania prymitywów.
\end{itemize}
Aktualizacja stanu renderara elementem jest odzwierciedlana aktualizacją stani GPU elementu.
Przykładowo wywołanie metody \textit{update\_geometry()} aktualizuje strukturę vertex\_stream\_element elementów prymitywów offsetami wierzchołków i indeksów prymitywu w strumieniu wierzchołków.


// HIRO 

// HIRO Grupy poleceń rysowania batches

// HIRO Stan renderowania renderer\_state

// HIRO Graf renderowania render\_graph

// HIRO Przebieg renderowania render\_pass

// HIRO Program shaderów shader\_program 

// HIRO Generator shaderów shader\_generator

// HIRO renderer

// HIRO Renderer przykładowych scen main
