...
Siatka jest tożsama z geometrią pojedyńczego modelu przygotowanego w programie do modelowania 3D.
...
Zasób atributu wierzchołka \textit{asset\_vertex\_attribute} reprezentuje dane pojedyńczego atrybutu przechowywane w postaci tablicy komponentów, których typ to \textit{uint32\_t}, \textit{vec2}, \textit{vec3} lub \textit{vec4}.
...
\begin{figure}[htbp]
	...
	\caption{Przykładowa tekstura dla czcionki Go-Mono \cite{GOMONOFONT}}
	\label{font_texture}
\end{figure}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Obiekty biorące udział w procesie konwersji sceny wraz z przepływem danych}
	\label{scene_process}
\end{figure}
