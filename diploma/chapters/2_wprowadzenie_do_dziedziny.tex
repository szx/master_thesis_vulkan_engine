...
\begin{itemize}
	...
	\item łańcucha wymiany (\textit{VkSwapchain}).
\end{itemize}
...
Kolejki są używane do wykonywania na urządzeniu fizycznym poleceń zawartych w buforach poleceń wysłanych do kolejki funkcją \textit{vkQueueSubmit()}.
...
\subsubsection{Łańcuch wymiany}

Łańcuch wymiany to tablica prezentowalnych obrazów należących do powierzchni okna.
...
wymiany funkcją \textit{vkAcquireNextImageKHR()}, wyrenderować do niego scenę oraz ponownie dodać go do kolejki obrazów do prezentacji w łańcuchu wymiany funkcją \textit{vkQueuePresentKHR()}.

Okno wyświetla tylko jeden prezentowalny obraz na raz, ale istnieje możliwość umieszczania kilku obrazów w kolejce do
prezentacji. Aktualnie prezentowany obraz jest zwany \textit{buforem przednim} (front buffer), a reszta obrazów w kolejce od prezentowania jest zwana \textit{buforami tylnymi} (back buffers).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1cm]
		...
		\node (PresentEvent) [function] at (390:4cm) {Prezentacja obrazu};
		...
		\draw [arrow, postaction={decorate,decoration={text along path,raise=1mm,text align=center,reverse path,text={Silnik prezentacji}}}] (305:16mm) arc (305:486:16mm);
...
// HIRO FIRO, MAILBOX etc.


\subsubsection{Diagram inicjalizacji}

/// HIRO diagram inicjalizacji


\subsection{Bufory poleceń}

// TODO surface, urządzenie fizyczne i logiczne, diagram state
