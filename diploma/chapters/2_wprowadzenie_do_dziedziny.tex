...
Oba rodzaje zasobów muszą posiadać dowiązaną alokację pamięci \textit{VkDeviceMemory}.
// HIRO pamięć

Odczyt i zapis danych z zasobu wymaga utworzenia widoku: \textit{VkBufferView} lub \textit{VkImageView}.
// HIRO widok
...
\begin{itemize}
	...
	\item \textit{UNIFORM\_BUFFER}: może być buforem uniform opisanym deskryptorem typu \textit{UNIFORM\_BUFFER};
	\item \textit{STORAGE\_BUFFER}: może być buforem magazynowym opisanym deskryptorem typu \textit{STORAGE\_BUFFER};
	...
	\item \textit{INDIRECT\_BUFFER}: może być używany jako bufor poleceń rysowania pośredniego w poleceniach \textit{vkCmdDrawIndirect()} i \textit{vkCmdDrawIndexedIndirect()}.
\end{itemize}
...
programistę do samodzielnego śledzenia stanu układu obrazów i wykonywania przejść na poprawny układ przed użyciem ich przez polecenia poprzez użycie odpowieniej bariery pamięci obrazu.


\subsubsection{Próbniki}

Teksele obrazy zwykle nie są bezpośrednio odczytywane w shaderach. Zamiast tego są one używane jako tekstury, do których dostęp odbywa się poprzez próbnik \textit{VkSampler} określający operacje filtrowania
// HIRO VkSampler próbkowanie, filtrowanie mag, min, mipmap
...
// TODO vkRenderPass - przebieg renderowania

Dołączenie koloru to widok obrazu używany przez  do zapisywania końcowych wartości kolorów podczas etapu mieszania kolorów potoku graficznego.

Dołączenie głębi/szablonu to widok obrazu używany przez potok graficzny do odczytywania i zapisywania wartości głebi/szablonu podczas testów głębi/szablonu w etapach wczesnych i późnych testów fragmentów potoku graficznego.
...
Przykładowo w scenie z samochodem jego cztery koła są renderowane przy pomocy czterech osobnych poleceń rysowania różniących się tylko pozycją, które mogą być 
HIRO
