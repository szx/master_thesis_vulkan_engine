...
Renderowanie w Vulkan wymaga wysłania do kolejki bufora poleceń wypełnionego poleceniami rysowania.

Vulkan 1.2 oferuje następujące warianty polecenia rysowania:
\begin{itemize}
	\item \textit{vkCmdDraw()}: polecenie rysowania, którego parametrami są:
	\begin{itemize}
	...
	\end{itemize}
	\item \textit{vkCmdDrawIndexed()}: polecenie rysowania HIRO
	\item \textit{vkCmdDrawIndirect()}: polecenie rysowania pośredniego HIRO
	\item \textit{vkCmdDrawIndexedIndirect()}: HIRO
	\item \textit{vkCmdDrawIndirectCount()}: HIRO
	\item \textit{vkCmdDrawIndexedIndirectCount()}: HIRO
\end{itemize}

// HIRO instancjonowanie, wielokrotne polecenia rysowania pośredniego,
...
// HIRO Potok graficzny to

// HIRO vkPipeline - potok, potok graficzny
...
// HIRO dynamiczne przebiegi renderowania
...
// HIRO draw call batching używając jednolitego bufora geometrii i poleceń rysowania pośredniego
